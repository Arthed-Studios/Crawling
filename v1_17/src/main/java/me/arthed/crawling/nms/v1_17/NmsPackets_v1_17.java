package me.arthed.crawling.nms.v1_17;
...
public class NmsPackets_v1_17 implements NmsPackets {

    private final int blockId = -8854;
    private final int floorBlockId = -8855;
    private final DataWatcher dataWatcher;

    public NmsPackets_v1_17() {
    ...
    public void spawnFakeBlocks(Player player, Block block, Block floorBlock, Material fakeFloorMaterial) {
        PacketPlayOutSpawnEntity spawnBlockPacket = new PacketPlayOutSpawnEntity(
                ...
                0, //pitch
                EntityTypes.C,
                net.minecraft.world.level.block.Block.getCombinedId(((CraftBlock)block).getNMS()), //material id
        ...
        PacketPlayOutSpawnEntity spawnBlockPacket2 = new PacketPlayOutSpawnEntity(
                ...
                0, //pitch
                EntityTypes.C,
                net.minecraft.world.level.block.Block.getCombinedId(((CraftBlock)floorBlock).getNMS()), //material id
                new Vec3D(0, 1, 0) // velocity
        );
        PacketPlayOutEntityMetadata blockMetadataPacket2 = new PacketPlayOutEntityMetadata(this.floorBlockId, this.dataWatcher, true);
        ((CraftPlayer)player).getHandle().b.sendPacket(spawnBlockPacket);
        ((CraftPlayer)player).getHandle().b.sendPacket(blockMetadataPacket);
        ((CraftPlayer)player).getHandle().b.sendPacket(spawnBlockPacket2);
        ((CraftPlayer)player).getHandle().b.sendPacket(blockMetadataPacket2);
    ...
    public void removeFakeBlocks(Player player) {
        ...
        ((CraftPlayer)player).getHandle().b.sendPacket(destroyOldBlockPacket);
        ((CraftPlayer)player).getHandle().b.sendPacket(destroyOldBlockPacket2);
    }
